...
namespace Nomad.Dialog.Editor
{
    public class DialogEditor : EditorWindow
    {
        ...
        [NonSerialized]
        private float rootNodeHeightOffset = 130;
        [NonSerialized]
        private float nodeHeightOffset = 190;
        [NonSerialized]
        ...
        {
            EditorGUILayout.BeginHorizontal();
            UnityEngine.Object obj = node.GetOuterChoiceAtIndex(outerChoiceIndex).GetInnerChoiceAtIndex(i);
            Choice choice = CreateInstance<Choice>();// = EditorGUILayout.ObjectField("", obj, typeof(Choice), true, GUILayout.MaxWidth(outerChoiceRect[nodeIndex][outerChoiceIndex].width - (OuterChoicestyle.padding.left + OuterChoicestyle.border.left) - 20)) as Choice;
            string buttonName = obj != null && ((Choice)obj).choiceText != "" ? ((Choice)obj).choiceText : "Add Choice";
                ...
                GUILayout.MaxWidth(outerChoiceRect[nodeIndex][outerChoiceIndex].width - (OuterChoicestyle.padding.left + OuterChoicestyle.border.left) - 20)))
            {
                ...
                choiceEditor.Show();
                choiceEditor.Init(node, outerChoiceIndex, i, obj as Choice);
            }
            if (obj == null) node.GetOuterChoiceAtIndex(outerChoiceIndex).SetInnerChoiceAtIndex(i, choice);
